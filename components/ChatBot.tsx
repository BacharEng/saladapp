...
const ChatBot: React.FC = () => {
  const { messages, pathHistory, addMessage, setPathHistory, popPathHistory } =
    useChatStore();
    ...
    <View style={styles.container}>
      <ScrollView>
        {messages.map((message: Message, index: number) => (
          <View key={index}>
            <Text style={{ fontSize: 17 }}>{message.text}</Text>
          </View>
        ))}
        {chatSteps[currentStep].options.map(
          (option: ChatOption, index: number) => (
            <>
              <Button
                onPress={() => {
